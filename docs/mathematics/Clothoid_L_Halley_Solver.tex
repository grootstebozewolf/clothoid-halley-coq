...
Throughout this paper we restrict attention to the \emph{monotone branch} of the residual, characterised by
\[
...
Halley's method reduces iteration count by 22\% on real railway data (2.28 vs.\ 2.92) and beats Newton on wall time across every implementation we measured, despite performing two additional moment integrals (\(S_{2c}, S_{2s}\)) per iteration. The fastest implementation is C\# at \SI{0.59}{\micro\second} per Halley solve; the slowest compiled runtime is Java at \SI{1.38}{\micro s}. NumPy's per-case overhead (\SI{33}{\micro s}) is dominated by Python interpreter dispatch rather than arithmetic.
...
The author used Anthropic's Claude (\texttt{claude-opus-4-7}) as a coding and editorial assistant during the preparation of this manuscript. The model assisted with the Coq proof script for Theorem~\ref{lst:coq} (the residual second-derivative identity), with the porting of the Python reference solver to the C\#, Java, and TypeScript implementations described in Section~\ref{sec:bench}, with the assembly of the ProRail data-fetch pipeline, and with copy-editing of the present text. All mathematical statements, formal proofs, benchmark measurements, and editorial decisions were verified and accepted by the author, who takes full responsibility for the contents of this paper.
