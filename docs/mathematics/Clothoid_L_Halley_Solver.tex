...
\end{verbatim}
A matching \texttt{solveNewtonL} is also supplied. The golden-vector regression suite (file \texttt{data/golden\_vectors.json}, 9{,}058 records exported from the Python reference) is executed by all three implementations; every case agrees with the reference within $10^{-9}$ metres, and Halley/Newton iteration counts match the reference exactly on 100\% of cases.
