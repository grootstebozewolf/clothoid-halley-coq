...
\usepackage{booktabs}
% siunitx removed for minimal build
\usepackage{graphicx}
\usepackage{hyperref}
...
We provide production-ready implementations in C\# (.NET 8), Java 21, and TypeScript (Node.js), all producing bit-identical results. Benchmarks on approximately 180 real ProRail transition curves (``Overgangsboog'') demonstrate that Halley's method consistently outperforms a comparable Newton variant, achieving convergence in an average of 5.3 iterations (vs.\ 7.8) with sub-4\,\si{\micro\second} solve times. The mathematical derivations are cross-verified with SymPy and the overall approach is situated within the current state-of-the-art, building upon the foundational work of Bertolazzi \& Frego while offering an explicit $L$-formulation with enhanced robustness guarantees.
\end{abstract}
...
\end{align}
All four integral-level derivatives of the moments ($P' = -T$, $Q' = R$, etc.) follow directly from differentiation under the integral sign and are verified symbolically with SymPy.
...
We have delivered a robust, well-documented, and multi-language Halley solver for the clothoid chord-length problem together with a complete academic reference. The implementation is already being integrated into the \texttt{allure-wkt} toolchain and a forthcoming PostgreSQL extension that will expose native \texttt{CLOTH OID} WKT support with on-the-fly $L$ solving.
...
\end{document}
>>>>>>> 30962ed (docs: add full academic paper + reproducible benchmark figures)
